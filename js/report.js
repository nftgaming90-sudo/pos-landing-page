...
window.previewNotaImage = (trxId) => {
    ...
    // 🔥 PERBAIKAN STRUKTUR HTML UNTUK CAPTURE
    let htmlContent = `
        <div id="wrapper-preview-rekap" class="flex flex-col items-center gap-4 w-full max-w-[320px] mx-auto p-4">
            <div id="struk-nota-capture" style="width:300px; background:#fff; padding:25px; color:#000; font-family:monospace; border-radius:8px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1);">
                <div style="text-align:center; font-weight:900; font-size:16px;">POSKITA STORE</div>
                <div style="text-align:center; font-size:10px; margin-bottom:10px;">NOTA #${trxId.toString().slice(-6)}</div>
            ...
            <div class="flex flex-col gap-2 w-full px-4 no-capture">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button id="btn-do-capture-nota" class="py-3 bg-slate-700 text-white rounded-xl font-black text-[9px] uppercase tracking-widest">💾 Simpan Galeri</button>
                    <button onclick="document.getElementById('section-struk').classList.remove('show-capture'); document.getElementById('section-struk').innerHTML='';" class="py-3 bg-rose-100 text-rose-600 rounded-xl font-black text-[9px] uppercase tracking-widest">Tutup</button>
                </div>
            </div>
        </div>
    `;
    ...
    printArea.innerHTML = htmlContent;
    printArea.classList.add('show-capture');
    printArea.style.display = 'flex'; // Paksa muncul
    ...
    document.getElementById('btn-share-nota').onclick = async () => {
        ...
        btn.innerText = "MENYIAPKAN...";
        try {
            // 🔥 Ambil ID yang benar: struk-nota-capture
            const area = document.getElementById('struk-nota-capture');
    ...
    document.getElementById('btn-do-capture-nota').onclick = async () => {
        ...
        try {
            ...
            const canvas = await html2canvas(area, { scale: 2, backgroundColor: "#ffffff" });
            const link = document.createElement('a');
            link.download = `Nota-${trxId.toString().slice(-6)}.png`;
            link.href = canvas.toDataURL("image/png");
            link.click();
